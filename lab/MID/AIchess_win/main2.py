...
def main():
    ...
    myai = MyAI(game.computer_team)

    # 主循环
    while True:
        # myai行动
        if not game.show_win and not game.show_draw and not game.AI_mode and game.get_player() == myai.team:
            if game.back_button.is_repeated():
                ...
                #chessboard.chessboard_map[cur_row][cur_col], chessboard.chessboard_map[nxt_row][nxt_col] =\
                #chessboard.chessboard_map[nxt_row][nxt_col], chessboard.chessboard_map[cur_row][cur_col]
                #chessboard.move_chess(nxt_row, nxt_col)
                # 清理「点击对象」
                ClickBox.clean()
                # 检测落子后，是否产生了"将军"功能
